...
// 새롭게 추가될 경우
  ...
  static Future<void> insertTBL_USER(String name, int age) async {
    print("name === > $name");
    final db = await getDatabase();
  ...
  static Future<int> updateUser(int userId, String name, int age) async {
    final db = await getDatabase();
    return await db.update('TBL_USER', {'name': name, 'age': age}, where: 'userId = ?', whereArgs: [userId]);
  }
